...
public class NotaController : ControllerBase
{
    ...
    [AllowAnonymous]
    [HttpGet("list-fecha", Name = "GetNotaListFecha")]
    [ProducesResponseType(typeof(IReadOnlyList<EListaPedidoFecha>), (int)HttpStatusCode.OK)]
    public async Task<ActionResult<IReadOnlyList<EListaPedidoFecha>>> ListarNotaFecha(
        [FromQuery] DateTime? fechaInicio,
        [FromQuery] DateTime? fechaFin,
        [FromQuery] int page = 1,
        [FromQuery] int pageSize = 50,
        CancellationToken cancellationToken = default)
    {
        ...
        return Ok(await _mediator.ListarFechaAsync(fechaInicio.Value, fechaFin.Value, page, pageSize, cancellationToken));
    }
    ...
    public async Task<IActionResult> RegistrarNotaPedidoConDetalle([FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        if (hasNotaObject)
        {
            ...
            return await GuardarOrdenConDetalleAsync(request, cancellationToken);
        }
    ...
    [AllowAnonymous]
    [HttpPost("guardar", Name = "GuardarNotaPedido")]
    [ProducesResponseType((int)HttpStatusCode.OK)]
    public async Task<IActionResult> GuardarNotaPedido([FromBody] NotaPedidoConDetalleRequest request, CancellationToken cancellationToken)
    {
        return await GuardarOrdenConDetalleLegacyAsync(request, cancellationToken);
    }
    ...
    private async Task<IActionResult> GuardarOrdenConDetalleAsync(NotaPedidoConDetalleRequest? request, CancellationToken cancellationToken)
    {
        ...
        var usarPayloadExtendido = await UspInsertarNotaBSoportaDatosDocumentoClienteAsync(cancellationToken);
        var vdataNota = usarPayloadExtendido
            ? BuildOrdenPayload(
                request.Nota,
                detalles,
                await ObtenerDatosClienteDocumentoConFallbackAsync(request.Nota.ClienteId, cancellationToken))
            : BuildOrdenPayloadLegacy(request.Nota, detalles, request.UsuarioId, request.IdOrden);
    ...
    private async Task<bool> UspInsertarNotaBSoportaDatosDocumentoClienteAsync(CancellationToken cancellationToken)
    {
        var connectionString = _configuration.GetConnectionString("DefaultConnection");
        if (string.IsNullOrWhiteSpace(connectionString))
        {
        ...
        const string sql = """
            SELECT OBJECT_DEFINITION(OBJECT_ID(N'dbo.uspinsertarNotaB'));
            """;
        ...
        try
        {
            ...
            await con.OpenAsync(cancellationToken);
            var definition = (await cmd.ExecuteScalarAsync(cancellationToken))?.ToString() ?? string.Empty;
            if (string.IsNullOrWhiteSpace(definition))
            {
                return true;
            }

            return definition.Contains("@c42", StringComparison.OrdinalIgnoreCase) ||
                   definition.Contains("@ClienteRazon", StringComparison.OrdinalIgnoreCase);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "No se pudo detectar la version de uspinsertarNotaB; se usara payload extendido.");
            return true;
        }
    }

    private async Task<IActionResult> GuardarOrdenConDetalleLegacyAsync(NotaPedidoConDetalleRequest? request, CancellationToken cancellationToken)
    {
        if (request?.Nota is null)
        {
            return BadRequest("NotaPedido requerida.");
        ...
        try
        {
            var usuarioIdConCaja = await ResolverUsuarioIdConCajaActivaAsync(
                request.UsuarioId,
                request.Nota.NotaUsuario,
                cancellationToken);
            var payload = BuildOrdenPayloadLegacy(request.Nota, detalles, usuarioIdConCaja, request.IdOrden);
            var resultado = await _mediator.RegistrarOrdenAsync(payload, cancellationToken);
            return Ok(new { resultado });
        }
        catch (SqlException ex)
        {
            _logger.LogError(ex, "Error SQL al guardar nota con formato legacy.");
            return BadRequest(new
            {
                ok = false,
                mensaje = "Error SQL al guardar la nota.",
                detalle = ex.Message
            });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error no controlado al guardar nota con formato legacy.");
            return StatusCode((int)HttpStatusCode.InternalServerError, new
            {
                ok = false,
                mensaje = "Error no controlado al guardar la nota.",
                detalle = ex.Message
            });
        }
    }

    private async Task<int?> ResolverUsuarioIdConCajaActivaAsync(
        int? usuarioId,
        string? usuarioAlias,
        CancellationToken cancellationToken)
    {
        ...
        const string sql = """
            ...
            WHERE c.CajaEstado = 'ACTIVO'
              AND (
                    UPPER(LTRIM(RTRIM(u.UsuarioAlias))) = UPPER(LTRIM(RTRIM(@UsuarioAlias)))
                 OR UPPER(LTRIM(RTRIM(ISNULL(c.CajaUsuario, '')))) = UPPER(LTRIM(RTRIM(@UsuarioAlias)))
              )
            ORDER BY c.CajaId DESC;
        ...
        try
        {
            ...
            cmd.Parameters.AddWithValue("@UsuarioId", usuarioId.GetValueOrDefault());
            cmd.Parameters.AddWithValue("@UsuarioAlias", (object?)usuarioAlias?.Trim() ?? string.Empty);
        ...
        {
            _logger.LogWarning(ex, "No se pudo resolver usuario con caja activa para guardar orden.");
        }

        return usuarioId;
    }

    private string BuildOrdenPayloadLegacy(
        NotaPedido nota,
        IReadOnlyList<DetalleNota> detalles,
        int? usuarioId,
        long? idOrden)
    {
        var documento = string.IsNullOrWhiteSpace(nota.NotaDocu) ? "BOLETA" : nota.NotaDocu!.Trim().ToUpperInvariant();
        var icbper = nota.ICBPER ?? 0m;
        var totalDetalle = detalles.Sum(x => x.DetalleImporte ?? 0m);
        var total = nota.NotaTotal ?? (totalDetalle + icbper);
        var tributario = CalcularTributacionDocumento(documento, total, icbper);

        var subtotal = nota.NotaSubtotal ?? tributario.SubTotal;
        var movilidad = nota.NotaMovilidad ?? 0m;
        var descuento = nota.NotaDescuento ?? 0m;
        var acuenta = nota.NotaAcuenta ?? 0m;
        var saldo = nota.NotaSaldo ?? total;
        var adicional = nota.NotaAdicional ?? 0m;
        var tarjeta = nota.NotaTarjeta ?? 0m;
        var pagar = nota.NotaPagar ?? total;
        var estado = string.IsNullOrWhiteSpace(nota.NotaEstado) ? "PENDIENTE" : nota.NotaEstado!;
        var companiaId = nota.CompaniaId ?? 0;
        var entrega = string.IsNullOrWhiteSpace(nota.NotaEntrega) ? "INMEDIATA" : nota.NotaEntrega!;
        var concepto = string.IsNullOrWhiteSpace(nota.NotaConcepto) ? "MERCADERIA" : nota.NotaConcepto!;
        var serie = string.IsNullOrWhiteSpace(nota.NotaSerie) ? "0001" : nota.NotaSerie!;
        var numero = string.IsNullOrWhiteSpace(nota.NotaNumero) ? "00000001" : nota.NotaNumero!;
        var ganancia = nota.NotaGanancia ?? 0m;
        var letras = Letras.enletras(total.ToString("N2", CultureInfo.InvariantCulture)) + "  SOLES";
        var docuAdicional = 0m;
        var docuHash = string.Empty;
        var estadoSunat = "PENDIENTE";
        var docuSubtotal = tributario.SubTotal;
        var docuIgv = tributario.Igv;
        var usuario = usuarioId.GetValueOrDefault() > 0 ? usuarioId!.Value : 7;
        var efectivo = nota.Efectivo ?? pagar;
        var vuelto = efectivo - pagar;
        if (vuelto < 0m) vuelto = 0m;
        var ordenId = idOrden.GetValueOrDefault() > 0 ? idOrden!.Value : (nota.NotaId > 0 ? nota.NotaId : 0);
        var docuGravada = tributario.Gravada;
        var docuDescuento = descuento;

        var headerFields = new[]
        {
            SanitizeSqlListField(documento),
            (nota.ClienteId ?? 1).ToString(CultureInfo.InvariantCulture),
            SanitizeSqlListField(nota.NotaUsuario),
            SanitizeSqlListField(nota.NotaFormaPago),
            SanitizeSqlListField(nota.NotaCondicion),
            SanitizeSqlListField(nota.NotaDireccion),
            SanitizeSqlListField(nota.NotaTelefono),
            Format2(subtotal),
            Format2(movilidad),
            Format2(descuento),
            Format2(total),
            ...
            Format2(tarjeta),
            Format2(pagar),
            SanitizeSqlListField(estado),
            companiaId.ToString(CultureInfo.InvariantCulture),
            SanitizeSqlListField(entrega),
            SanitizeSqlListField(concepto),
            SanitizeSqlListField(serie),
            SanitizeSqlListField(numero),
            Format2(ganancia),
            SanitizeSqlListField(letras),
            Format2(docuAdicional),
            SanitizeSqlListField(docuHash),
            SanitizeSqlListField(estadoSunat),
            Format2(docuSubtotal),
            Format2(docuIgv),
            usuario.ToString(CultureInfo.InvariantCulture),
            Format2(efectivo),
            Format2(vuelto),
        ...
        var payload = string.Join("|", headerFields) + "[";
        for (var i = 0; i < detalles.Count; i++)
        {
            var d = detalles[i];
            var detailFields = new[]
            {
                Convert.ToInt64(d.IdProducto ?? 0).ToString(CultureInfo.InvariantCulture),
                Format2(d.DetalleCantidad ?? 0m),
                SanitizeSqlListField(d.DetalleUm),
                SanitizeSqlListField(d.DetalleDescripcion),
                Format2(d.DetalleCosto ?? 0m),
                Format2(d.DetallePrecio ?? 0m),
                Format2(d.DetalleImporte ?? 0m),
                SanitizeSqlListField(string.IsNullOrWhiteSpace(d.DetalleEstado) ? "PENDIENTE" : d.DetalleEstado),
                Format4(d.ValorUM ?? 1m)
            };

            payload += string.Join("|", detailFields);
            if (i < detalles.Count - 1) payload += ";";
        }
    ...
    private static string SanitizeSqlListField(string? value)
    {
